...
for line in file:
    ...
    if line.startswith("*End Assembly"):
        ...
        SidePlate3 = filter(lambda x: x.name.startswith("SidePlate-3"), Instance.instances)[0]
        SidePlate4 = filter(lambda x: x.name.startswith("SidePlate-4"), Instance.instances)[0]
        FinPlate1 = filter(lambda x: x.name.startswith("FinPlate-1"), Instance.instances)[0]
        FinPlate2 = filter(lambda x: x.name.startswith("FinPlate-2"), Instance.instances)[0]
        SteelTube = filter(lambda x: x.name.startswith("SteelTubeV3-2"), Instance.instances)[0]
        ...
        SidePlate3NodeSet = SidePlate3.transnodeset(SidePlatePart.nodeset())
        SidePlate4NodeSet = SidePlate4.transnodeset(SidePlatePart.nodeset())
        ConcreteCoreNodeSet = ConcreteCore.nodeset()
        ...
        def BoundFunction(x):
            BoundX = -80.1 <= x[1] <= 75.1
            BoundY = 1159.9 <= x[2] <= 1160.1
            BoundZ = 104.9 <= x[3] <= 105.1 or 74.9 <= x[3] <= 75.1
            condition1 = BoundX and BoundY and BoundZ

            BoundX = 74.9 <= x[1] <= 75.1
            BoundY = 1159.9 <= x[2] <= 1160.1
            BoundZ = 74.9 <= x[3] <= 105.1
            condition2 = BoundX and BoundY and BoundZ
        ...
        FWSP3IS2 = InterNodes(SidePlate3.name, SidePlate3NodeSet, InternalStiffener2.name, InternalStiffener2NodeSet, BoundFunction, file2, [1, 2, 3], PairCounter)
        PairCounter = FWSP3IS2.pairnum
        ...
        FWSP4IS1 = InterNodes(SidePlate4.name, SidePlate4NodeSet, InternalStiffener1.name, InternalStiffener1NodeSet, BoundFunction, file2, [1, 2, 3], PairCounter)
        PairCounter = FWSP4IS1.pairnum
        ...
        FWIS1C = InterNodes(InternalStiffener1.name, InternalStiffener1NodeSet, SteelTube.name, SteelTubeNodeSet, BoundFunction, file2, [1, 2, 3], PairCounter, tol=0.5)
        ...
        FWIS2C = InterNodes(InternalStiffener2.name, InternalStiffener2NodeSet, SteelTube.name, SteelTubeNodeSet, BoundFunction, file2, [1, 2, 3], PairCounter, tol=0.5)
        ...
        FWSP3C = InterNodes(SidePlate3.name, SidePlate3NodeSet, SteelTube.name, SteelTubeNodeSet, BoundFunction, file2, [1, 2, 3], PairCounter)
        PairCounter = FWSP3C.pairnum
        FWSP3C.show()
